...
<body class="bg-slate-50 text-slate-900 flex min-h-screen">
    <aside class="w-64 bg-indigo-900 text-indigo-100 flex flex-col p-6 space-y-8 sticky top-0 h-screen">
        ...
        <nav class="flex-1 space-y-2">
            ...
            <a href="{{ route('admin.transactions.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.transactions.*') ? 'bg-indigo-800 text-white' : '' }} rounded-xl font-bold transition">Transaksi</a>
        </nav>
        <form action="{{ route('admin.logout') }}" method="POST" class="mt-auto">
            @csrf
            <button type="submit" class="w-full px-4 py-3 bg-white text-indigo-900 rounded-2xl font-bold hover:bg-slate-100 transition">Logout</button>
        </form>
    </aside>
